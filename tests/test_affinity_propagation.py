...
class TestAffinityPropagation(unittest.TestCase):
	...
	def run_single_hap_run(self, input, predicted_expected, tmp_out):
		...
		# print(df)
		gr = PyRanges(df)
		runs = cluster_by_haplotype(gr, seed=42, iterations=50, dampings=[0.5, 0.6, 0.7, 0.8, 0.9], max_missing=5, min_iterations=5)
	...
	def test_cluster_by_haplotype_long(self):
		...
		self.assertAlmostEqual(best.n_haplotypes, 18)
		print(best.as_df())
		...
		self.assertAlmostEqual(best.n_haplotypes, 53)
		print(best.as_df())
